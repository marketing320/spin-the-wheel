<div>
    ...
    @unless ($campaign)
        <div class="glass rounded-2xl px-6 py-10 text-center">
            ...
            <p class="mt-3 text-slate-300">No active campaign yet. Activate a campaign to configure its wheel.</p>
    ...
    @else
        <form wire:submit="save" class="card space-y-8">
            {{-- Appearance --}}
            <div>
                <h3 class="mb-4 text-sm font-semibold uppercase tracking-wider text-slate-400">Appearance</h3>
                <div class="grid gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        @error('label_style') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        ...
                        @error('background_style') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="label">Pointer color</label>
                        <div class="flex items-center gap-3">
                            <input type="color" wire:model="pointer_color"
                                class="h-10 w-14 shrink-0 cursor-pointer rounded-lg border border-white/10 bg-transparent">
                            <input type="text" wire:model="pointer_color" class="field" maxlength="9">
                        </div>
                        @error('pointer_color') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="label">Hub logo</label>
                        <input type="text" wire:model="hub_logo" class="field" maxlength="32"
                            placeholder="🎡">
                        <p class="mt-1 text-xs text-slate-500">Emoji or short text shown at the center of the wheel.</p>
                        @error('hub_logo') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
            ...
            {{-- Motion & effects --}}
            <div>
                <h3 class="mb-4 text-sm font-semibold uppercase tracking-wider text-slate-400">Motion &amp; effects</h3>
                <div class="grid gap-5 sm:grid-cols-2">
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <span class="w-16 shrink-0 text-right text-sm font-semibold text-white">{{ $animation_duration_ms }}</span>
                        </div>
                        <p class="mt-1 text-xs text-slate-500">How long a spin animation lasts (3000–15000 ms).</p>
                        @error('animation_duration_ms') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <input type="range" min="0" max="100" step="1"
                                ...
                                class="h-2 w-full cursor-pointer accent-brand-500">
                            <span class="w-16 shrink-0 text-right text-sm font-semibold text-white">{{ $glow_intensity }}</span>
                        </div>
                        <p class="mt-1 text-xs text-slate-500">Neon glow around the wheel segments (0–100).</p>
                        @error('glow_intensity') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <input type="range" min="0" max="100" step="1"
                                ...
                                class="h-2 w-full cursor-pointer accent-brand-500">
                            <span class="w-16 shrink-0 text-right text-sm font-semibold text-white">{{ $three_intensity }}</span>
                        </div>
                        <p class="mt-1 text-xs text-slate-500">Lighting, particles and 3D richness (0–100).</p>
                        @error('three_intensity') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
                    </div>
                    <div class="flex items-end">
                        ...
                        @error('sound_enabled') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>

            <div class="flex justify-end gap-2 border-t border-white/10 pt-5">
