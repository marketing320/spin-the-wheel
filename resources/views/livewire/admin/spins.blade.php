                ...
                <tbody class="divide-y divide-white/5">
                    @forelse ($spins as $spin)
                        <tr class="hover:bg-white/5">
                            <td class="px-4 py-3 font-mono text-xs text-slate-400">#{{ $spin->id }}</td>
                            <td class="px-4 py-3 text-slate-300">{{ $spin->campaign?->name ?? '—' }}</td>
                            <td class="px-4 py-3">
                                <div class="font-semibold text-white">{{ $spin->player?->email ?? '—' }}</div>
                            ...
                            <td class="px-4 py-3">
                                @if ($spin->prize)
                                    <div class="flex items-center gap-2">
                                        <span class="text-slate-300">{{ $spin->prize->name }}</span>
                            ...
                            <td class="px-4 py-3">
                                @php
                                    $statusStyles = [
                                        'completed' => 'bg-emerald-500/20 text-emerald-300 ring-1 ring-emerald-400/30',
                                        'active' => 'bg-amber-500/20 text-amber-300 ring-1 ring-amber-400/30',
                                        'expired' => 'bg-rose-500/20 text-rose-300 ring-1 ring-rose-400/30',
                                        'failed' => 'bg-rose-500/20 text-rose-300 ring-1 ring-rose-400/30',
                                        'pending' => 'bg-slate-500/20 text-slate-300 ring-1 ring-slate-400/30',
                                    ];
                                    $statusCls = $statusStyles[$spin->status] ?? $statusStyles['pending'];
                                @endphp
                                <span class="pill {{ $statusCls }}">{{ ucfirst($spin->status) }}</span>
                            </td>
                            <td class="px-4 py-3 text-xs text-slate-400">{{ $spin->started_at?->format('M j, Y H:i') ?? '—' }}</td>
                            <td class="px-4 py-3 text-xs text-slate-400">{{ $spin->completed_at?->format('M j, Y H:i') ?? '—' }}</td>
                        </tr>
                    @empty
