<div>
    ...
    @if (! $campaign)
        <div class="glass rounded-2xl p-8 text-center">
            ...
            <h3 class="mt-3 text-lg font-bold text-white">No active campaign</h3>
    ...
    @else
        <form wire:submit="save" class="glass max-w-2xl space-y-5 rounded-2xl p-6">
            <div>
                ...
                <div class="text-base font-semibold text-white">{{ $campaign->name }}</div>
            </div>
            ...
            <div>
                ...
                @error('location_name') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div>
                    ...
                    @error('latitude') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="label">Longitude</label>
                    <input type="number" step="any" wire:model="longitude" placeholder="-180 to 180" class="field">
                    @error('longitude') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
                </div>
            </div>
            ...
            <div>
                ...
                @error('radius_meters') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
            </div>
            ...
            <div>
                ...
                @error('blocked_message') <p class="mt-1 text-sm text-rose-400">{{ $message }}</p> @enderror
            </div>
